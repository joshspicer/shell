...
from shell.policy import load_host_keys, get_policy_class, check_policy_setting
from shell.utils import (
    to_ip_address,
    parse_origin_from_url,
    is_valid_encoding,
)
from shell._version import __version__
...
define(
    "policy", default="warning", help="Missing host key policy, reject|autoadd|warning"
)
...
define("delay", type=float, default=30, help="The delay to call recycle_worker")
define(
    "maxconn",
    type=int,
    default=20,
...
def get_app_settings(options):
    ...
    settings = dict(
        template_path=os.path.join(base_dir, "shell", "templates"),
        static_path=os.path.join(base_dir, "shell", "static"),
        websocket_ping_interval=options.wpintvl,
        debug=options.debug,
        cookie_secret=os.getenv("JWT_SIGNING_KEY", "insecure"),
...
def get_font_filename(font, font_dir):
    filenames = {
        ...
        if not f.startswith(".") and os.path.isfile(os.path.join(font_dir, f))
    }
    if font:
